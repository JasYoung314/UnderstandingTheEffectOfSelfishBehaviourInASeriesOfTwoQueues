...
class Player:
	def __init__(self,arrival_date,player_ID):
		...
		self.total_cost=0
		self.expected_cost=[]
		self.exit_location=False

	def make_decision(self,beta,Stations,Converted,exit_list,warm_period):
		k=0
		while k<len(Stations) and self.location<len(Stations):
			if Converted==1:
				if len(Stations[self.location].queue)>=len(Stations[self.location].servers_end):
					if beta[self.location]>len(Stations[self.location].queue):
						self.skip.append("False")
						return
					else:
						self.skip.append("True")
						self.cost.append(Stations[self.location].beta)
						self.server_used.append('N/A')
						self.service_time.append(0)
						self.service_start_date.append("N\A")
						self.wait.append(0)
						self.service_end_time.append("N/A")
						self.decision_date.append(self.decision_date[-1])
						self.total_cost+=self.cost[-1]
						self.expected_cost.append((1/Stations[self.location].service_rate))
						if self.arrival_date>warm_period:
							Stations[self.location].skip_count+=1
						self.location+=1
				else:
					if int(beta[self.location]/len(Stations[self.location].servers_end))>len(Stations[self.location].queue):
						self.skip.append("False")
						return
					else:
						self.skip.append("True")
						...
						self.decision_date.append(self.decision_date[-1])
						self.total_cost+=self.cost[-1]
						self.expected_cost.append((len(Stations[self.location].queue)+1)/(len(Stations[self.location].servers_end)*Stations[self.location].service_rate))
						if self.arrival_date>warm_period:
							Stations[self.location].skip_count+=1
						self.location+=1
			else:
...
def G_Sim(lmbda=False,mu=False,No_servers=False,skip_cost=False,Simulation_Time=False,Policy=False,warm_period=0,Print_Option=True,exit_list=False):
	...
	while t<Simulation_Time:
		...
		Players[decision_maker_ID].make_decision(N_policy,Stations,Policy_conversion,exit_list,warm_period)
		...
		#checks if a player exits the system.
		if not exit_list==False:
			if Players[decision_maker_ID].location<n:
				if exit_dist()<exit_list[Players[decision_maker_ID].location]:
					poss_decision_maker.pop(poss_decision_maker.index(decision_maker_ID))
					Players[decision_maker_ID].in_sys=False
					Players[decision_maker_ID].exit_location=Players[decision_maker_ID].location
					k=0
					while k<n+1-len(Players[decision_maker_ID].decision_date):
						Players[decision_maker_ID].cost.append(0)
						Players[decision_maker_ID].wait.append(0)
						Players[decision_maker_ID].service_time.append(0)
						k+=1

		#Removes players whove made thier final decision.
		if len(Players[decision_maker_ID].decision_date)==n+1 and Players[decision_maker_ID].in_sys==True:
			poss_decision_maker.pop(poss_decision_maker.index(decision_maker_ID))

		#print "%.2f Percent Complete" %(100*t/Simulation_Time)

	#Adds completed players to a new list
	completed_players=[]
	for e in Players:
		if e.in_sys==False and e.arrival_date>warm_period:
	...
	if not len(completed_players)==0:
		...
		cost_per_player=[e.total_cost for e in completed_players]
		ave_cost_per_player=sum(cost_per_player)/len(cost_per_player)
		...
		if not Print_Option:
			...
			return ave_Util,ave_cost_per_player
		...
		print "Mean Cost per Player      : ",ave_cost_per_player
		print ""
		print "Station Stats"
	...
	if input("Output data to csv (True/False)? "):
		...
		while k<len(Players):
			...
			outrow.append(Players[k].exit_location)
			h=0
			while h<n+1:
...
class Worker(multiprocessing.Process):
	...
	def __init__(self, work_queue, data_queue,lmbda,mu,c,skip_cost,simulation_time,warmup_period,work,Policy,exit_list):
	...
	def run(self):
		...
		while k<self.work:
			...
			data=G_Sim(self.lmbda,self.mu,self.c,self.skip_cost,self.Sim_Time,self.policy,self.warm,False,self.exit_list)
...
def pp_G_Sim(N,lmbda,mu,c,skip_cost,simulation_time,warmup_period,Policy=False,exit_list=False):
	Util_list=[]
	cost_list=[]
	Dummy=[]
	num_jobs = N
	...
	while k<num_processes:
		work_load.append(int(num_jobs/num_processes))
		k+=1
	k=0
	while k<num_jobs%num_processes:
		work_load[k]+=1
		k+=1
	...
	for i in range(num_processes):
		worker = Worker(work_queue, data_queue,lmbda,mu,c,skip_cost,simulation_time,warmup_period,work_load[i],Policy,exit_list)
		worker.start()

	# Collect the results off the queue
	results = []
	for i in range(num_jobs):
		Dummy.append(data_queue.get())

	k=0
	while k< len(Dummy):
		Util_list.append(Dummy[k][0])
		cost_list.append(Dummy[k][1])
		k+=1
	if N==0:
		return 0,0
	else:
		return Util_list,sum(cost_list)/len(cost_list),min(cost_list),max(cost_list),cost_list

def Price_of_Anarchy(Iterations,lmbda,mu,c,skip_cost,Simulation_Time,warm_period,Runs=multiprocessing.cpu_count(),exit_list=False,Failed_runs=False):

		#Calls the solver function
		data=Policy_Random_v45.Solver(Iterations,lmbda,mu,c,skip_cost,Simulation_Time,warm_period,Runs,Failed_runs,exit_list)

		#Stores data
		nash_list=data[2]
		opt_list=data[5]

		#Finds the standard deviation of all the lists
		POA_list=[]
		dummy_list=[]
		for e in nash_list:
			dummy_list.append((e-data[1])**2)

		nash_stnd=(sum(dummy_list)/len(dummy_list))**0.5

		dummy_list=[]
		for e in opt_list:
			dummy_list.append((e-data[4])**2)

		opt_stnd=(sum(dummy_list)/len(dummy_list))**0.5
		...
		POA_ave=sum(POA_list)/len(POA_list)

		for e in POA_list:
			dummy_list.append((e-POA_ave)**2)

		POA_stnd=(sum(dummy_list)/len(dummy_list))**0.5

		#Returns the relevant values
		POA=data[1]/data[4]

		return POA,nash_stnd,opt_stnd,POA_stnd,data[1],data[4],data[3]
